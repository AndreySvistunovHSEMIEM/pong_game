    ...
    while (row <= 6) {
        ...
        while (col <= 79) {
            if (row == 1 || row == 5) {
                ...
                (col == 15) ? printw("User 1: %d", score_first) : printw("User 2: %d", score_second);
                col = (col == 15) ? ((score_first > 9) ? col + 9 : col + 8)
                                  : ((score_second > 9) ? col + 9 : col + 8);
            } else if (row == 3 && col == 55) {
                printw("User 2: %d", score_second);
                col += 8;
...
int main() {
    ...
    while (score_first < 21 && score_second < 21) {
        ...
        if ((x_ball == 2 && (y_ball == rocket_l || y_ball == rocket_l - 1 || y_ball == rocket_l + 1)) ||
            (x_ball == 79 && (y_ball == rocket_r || y_ball == rocket_r - 1 || y_ball == rocket_r + 1))) {
            x_ball = (x_ball == 2) ? x_ball + 1 : x_ball - 1;
        }
